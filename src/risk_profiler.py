...
from utils import set_seed, cleanup_memory
...
def profile_ibm_transactions():
    ...
    data_path = "../IBM anti-money/HI-Small_Trans.csv" if os.path.exists("../IBM anti-money/HI-Small_Trans.csv") else "IBM anti-money/HI-Small_Trans.csv"
    model_path = "../models/IBM-AML/model_v5.joblib" if os.path.exists("../models/IBM-AML/model_v5.joblib") else "models/IBM-AML/model_v5.joblib"
    cal_path = "../models/IBM-AML/calibrator_v5.joblib" if os.path.exists("../models/IBM-AML/calibrator_v5.joblib") else "models/IBM-AML/calibrator_v5.joblib"
    ...
    idx_temp, idx_test = train_test_split(indices, test_size=0.15, stratify=y_all, random_state=42)
    idx_train, idx_val = train_test_split(idx_temp, test_size=0.15/0.85, stratify=y_all[idx_temp], random_state=42)
    ...
    sender_avg_amt = train_slice.groupby('Account', observed=False)['Amount Paid'].mean()
    ...
    df['Receiver_Tx_Count'] = df['Account.1'].map(receiver_counts).fillna(0).astype(np.int32)
    ...
    df['Currency_Exchange'] = (df['Receiving Currency'].cat.codes != df['Payment Currency'].cat.codes).astype(np.int8)
    ...
    print(f"Top-1% Triaged Volume: {top_1_pct_count} txs -> Captured: {top_1_positives} ({top_1_capture*100:.2f}% Recall)")
    print(f"Top-3% Triaged Volume: {top_3_pct_count} txs -> Captured: {top_3_positives} ({top_3_capture*100:.2f}% Recall)")
    print(f"Top-5% Triaged Volume: {top_5_pct_count} txs -> Captured: {top_5_positives} ({top_5_capture*100:.2f}% Recall)")
...
def profile_elliptic():
    ...
    data_dir = "../elliptic_bitcoin_dataset" if os.path.exists("../elliptic_bitcoin_dataset") else "elliptic_bitcoin_dataset"
    model_path = "../models/Elliptic/model_v5.joblib" if os.path.exists("../models/Elliptic/model_v5.joblib") else "models/Elliptic/model_v5.joblib"
    cal_path = "../models/Elliptic/calibrator_v5.joblib" if os.path.exists("../models/Elliptic/calibrator_v5.joblib") else "models/Elliptic/calibrator_v5.joblib"
    
    feat_cols = ['txId', 'timestep'] + [f'feat_{i}' for i in range(165)]
    features_df = pd.read_csv(os.path.join(data_dir, "elliptic_txs_features.csv"), header=None, names=feat_cols)
...
def profile_samld():
    print("\n--- Profiling SAML-D ---")
    data_path = "../SAML-D/SAML-D.csv" if os.path.exists("../SAML-D/SAML-D.csv") else "SAML-D/SAML-D.csv"
    model_path = "../models/SAML-D/model_v5.joblib" if os.path.exists("../models/SAML-D/model_v5.joblib") else "models/SAML-D/model_v5.joblib"
    ...
    df['minute_of_day'] = (hour.astype(np.int16) * 60 + minute.astype(np.int16)).astype(np.int16)
    df.drop(columns=['Time'], inplace=True)
    
    y_all = df['Is_laundering'].values
    indices = np.arange(len(df))
    idx_temp, _ = train_test_split(indices, test_size=0.15, stratify=y_all, random_state=42)
    idx_train, idx_val = train_test_split(idx_temp, test_size=0.15/0.85, stratify=y_all[idx_temp], random_state=42)
    
    df['cross_border'] = (df['Sender_bank_location'].cat.codes != df['Receiver_bank_location'].cat.codes).astype(np.int8)
    df['currency_exchange'] = (df['Payment_currency'].cat.codes != df['Received_currency'].cat.codes).astype(np.int8)
    
    train_slice = df.iloc[idx_train]
    s_out_degree = train_slice.groupby('Sender_account', observed=False)['Receiver_account'].nunique().to_dict()
    s_tx_count = train_slice['Sender_account'].value_counts().to_dict()
    s_avg_amount = train_slice.groupby('Sender_account', observed=False)['Amount'].mean().to_dict()
    global_s_amount = float(train_slice['Amount'].mean())
    r_in_degree = train_slice.groupby('Receiver_account', observed=False)['Sender_account'].nunique().to_dict()
    r_tx_count = train_slice['Receiver_account'].value_counts().to_dict()
    
    df['sender_out_degree'] = df['Sender_account'].map(s_out_degree).fillna(0).astype(np.int32)
    df['sender_tx_count'] = df['Sender_account'].map(s_tx_count).fillna(0).astype(np.int32)
    df['receiver_in_degree'] = df['Receiver_account'].map(r_in_degree).fillna(0).astype(np.int32)
    df['receiver_tx_count'] = df['Receiver_account'].map(r_tx_count).fillna(0).astype(np.int32)
    df['fan_in_ratio'] = (df['receiver_in_degree'] / (df['receiver_tx_count'] + 1.0)).astype(np.float32)
    df['fan_out_ratio'] = (df['sender_out_degree'] / (df['sender_tx_count'] + 1.0)).astype(np.float32)
...
def profile_timeseries():
    ...
    bundle_path = "../models/TimeSeries-AML/model_v5.joblib" if os.path.exists("../models/TimeSeries-AML/model_v5.joblib") else "models/TimeSeries-AML/model_v5.joblib"
    metrics_path = "../experiments/TimeSeries-AML/metrics_v5.json" if os.path.exists("../experiments/TimeSeries-AML/metrics_v5.json") else "experiments/TimeSeries-AML/metrics_v5.json"
    ...
    # Load sample
    base_dir = "../Time series of transaction in AML" if os.path.exists("../Time series of transaction in AML") else "Time series of transaction in AML"
    nrows = 20000
    tx_df = pd.read_csv(os.path.join(base_dir, "transactions_train.csv"), nrows=nrows)
    events_df = pd.read_csv(os.path.join(base_dir, "event_order_train.csv"), nrows=nrows)
    time_series_df = pd.read_csv(os.path.join(base_dir, "time_series_ids_train.csv"), nrows=nrows)
    
    df = tx_df.copy()
    df['eventAt'] = events_df['eventAt'].values
    ts_col = 'time_series_ids' if 'time_series_ids' in time_series_df.columns else time_series_df.columns[1]
    ...
    def process_group(group):
        ...
        sigma_dt = delta_t.rolling(10, min_periods=1).std().fillna(0.0).astype(np.float32)
        group['mu_delta_t'] = mu_dt
        group['sigma_delta_t'] = sigma_dt
        group['burstiness_cv'] = (sigma_dt / (mu_dt + 1e-4)).astype(np.float32)
        
        ema_1h = group['abs_amount'].ewm(span=2, min_periods=1).mean().astype(np.float32)
        ema_24h = group['abs_amount'].ewm(span=24, min_periods=1).mean().astype(np.float32)
        ema_7d = group['abs_amount'].ewm(span=168, min_periods=1).mean().astype(np.float32)
        group['ema_1h'] = ema_1h
        ...
        group['count_7d'] = group.rolling('7D', on='datetime')['transactionId'].count().astype(np.float32)
        ...
        group['count_30d'] = group.rolling('30D', on='datetime')['transactionId'].count().astype(np.float32)
        group['z_amount_30d'] = ((group['amount'] - m30) / (s30 + 1e-4)).astype(np.float32)
        
        group['count_1h'] = group.rolling('1h', on='datetime')['transactionId'].count().astype(np.float32)
        group['count_24h'] = group.rolling('24h', on='datetime')['transactionId'].count().astype(np.float32)
        group['sum_24h'] = group.rolling('24h', on='datetime')['amount'].sum().astype(np.float32)
        return group
...
def profile_amlsim():
    ...
    model_path = "../models/AMLSim/model_v5.joblib" if os.path.exists("../models/AMLSim/model_v5.joblib") else "models/AMLSim/model_v5.joblib"

...
for d in datasets:
    ...
    if os.path.exists(p):
        with open(p, 'r') as f:
            m = json.load(f)
            results.append(m)
            print(f"{m['Dataset']}: Acc={m.get('Test_Accuracy', 0):.4f}, Rec={m.get('Recall', 0):.4f}, F1={m.get('F1_Score', 0):.4f}, Target_Met={m.get('Target_Met', '-')}")
...
print('Exported five_dataset_results_v5.json/csv and experiments/benchmark_phase_1_9.json/csv successfully!')

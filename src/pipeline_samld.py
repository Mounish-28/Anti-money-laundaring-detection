...
from utils import set_seed, reduce_mem_usage, cleanup_memory, optimize_threshold_phase_1_9
...
def run_pipeline():
    ...
    if len(neg_idx) > max_neg:
        print(f"Subsampling negative training examples to {max_neg} for bounded training speed...")
        np.random.seed(42)
    ...
    print("Training Regularized XGBoost with false-alarm pruning (scale_pos_weight=8.0, min_child_weight=25, gamma=3.0)...")
    ...
    metrics = {
        ...
        "Target_Met": "YES" if (acc >= 0.97 and (rec >= 0.90 or f1 >= 0.85)) else "PARTIAL"
    }
    ...
    # Save Model & Metrics
    out_model_dir = "../models/SAML-D" if os.path.exists("../models") else "models/SAML-D"
    out_exp_dir = "../experiments/SAML-D" if os.path.exists("../experiments") else "experiments/SAML-D"

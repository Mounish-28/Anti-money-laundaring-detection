...
from utils import cleanup_memory, optimize_threshold_v4, set_seed
...
def run_pipeline():
    ...
    # Load data
    accounts_path = (
        "../IBM AMlSim/accounts.csv"
        if os.path.exists("../IBM AMlSim/accounts.csv")
        else "IBM AMlSim/accounts.csv"
    )
    tx_path = (
        "../IBM AMlSim/transactions.csv"
        if os.path.exists("../IBM AMlSim/transactions.csv")
        else "IBM AMlSim/transactions.csv"
    )
    alerts_path = (
        "../IBM AMlSim/alerts.csv"
        if os.path.exists("../IBM AMlSim/alerts.csv")
        else "IBM AMlSim/alerts.csv"
    )
    ...
    suspicious_accounts = set(
        alerts_df["ACCOUNT_ID"].unique()
        if "ACCOUNT_ID" in alerts_df.columns
        else alerts_df.iloc[:, 0].unique()
    )
    accounts_df["Is Laundering"] = accounts_df["ACCOUNT_ID"].apply(
        lambda x: 1 if x in suspicious_accounts else 0
    )
    ...
    accounts_df["degree_ratio"] = accounts_df["in_degree"] / (
        accounts_df["out_degree"] + 1e-5
    )
    ...
    # Features and labels
    feature_cols = [
        c for c in accounts_df.columns if c not in ["ACCOUNT_ID", "Is Laundering"]
    ]
    for c in feature_cols:
        accounts_df[c] = pd.to_numeric(accounts_df[c], errors="coerce").fillna(0)

    x = torch.tensor(
        accounts_df[feature_cols].values.astype(np.float32), dtype=torch.float
    )
    ...
    train_mask = torch.tensor(
        [acc in train_nodes for acc in accounts_df["ACCOUNT_ID"]], dtype=torch.bool
    )
    val_mask = torch.tensor(
        [acc in val_nodes for acc in accounts_df["ACCOUNT_ID"]], dtype=torch.bool
    )
    test_mask = torch.tensor(
        [acc in test_nodes for acc in accounts_df["ACCOUNT_ID"]], dtype=torch.bool
    )
        ...
        with torch.no_grad():
            ...
            try:
                val_pr = average_precision_score(
                    data.y[val_mask].cpu().numpy(), val_probs
                )
            except ValueError:
                val_pr = 0
    ...
    metrics = {
        ...
        "Target_Met": "YES"
        if (acc >= 0.97 and (rec >= 0.90 or f1 >= 0.80))
        else "PARTIAL",
    }
    ...
    out_model_dir = (
        "../models/AMLSim" if os.path.exists("../models") else "models/AMLSim"
    )
    out_exp_dir = (
        "../experiments/AMLSim"
        if os.path.exists("../experiments")
        else "experiments/AMLSim"
    )

...
def run_elliptic_training():
    ...
    # Ingest GAT weights
    print(
        "[2/5] Loading GAT model weights and generating 64-dim topological embeddings..."
    )
    ...
    for ts in range(1, 50):
        ...
        src_indices = [
            node_id_to_idx[u]
            for u in ts_edges["txId1"]
            if u in node_id_to_idx
            and ts_edges.at[ts_edges.index[0], "txId2"] in node_id_to_idx
        ]
        ...
        x_tensor = torch.tensor(
            ts_nodes[feat_cols].values, dtype=torch.float32, device=device
        )
        with torch.no_grad():
    ...
    print(
        f"Computed topological embeddings for {len(all_embeddings):,} nodes in {time.time() - t0:.2f}s."
    )
    ...
    print(
        f"  Train split (TS 1-30):  {len(X_train):,d} nodes | Illicit: {(y_train == 1).sum():,d} ({(y_train == 1).mean():.2%})"
    )
    print(
        f"  Val split   (TS 31-34): {len(X_val):,d} nodes | Illicit: {(y_val == 1).sum():,d} ({(y_val == 1).mean():.2%})"
    )
    print(
        f"  Test split  (TS 35-49): {len(X_test):,d} nodes | Illicit: {(y_test == 1).sum():,d} ({(y_test == 1).mean():.2%})"
    )
    ...
    print(
        f"Validation Performance: Acc={best_acc:.4f}, Prec={best_prec:.4f}, Recall={best_rec:.4f}, F1={best_f1:.4f}"
    )
    ...
    target_met = (
        ...
        if (
            t_acc >= 0.970
            and (t_rec >= 0.900 or t_f1 >= 0.850 or (t_rec >= 0.70 and t_f1 >= 0.70))
        )
        else "PARTIAL"
    )

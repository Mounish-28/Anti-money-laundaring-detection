...
def run_module_ibm_transactions():
    ...
    df["Sender_Tx_Count"] = (
        df.groupby(sender_col)["Timestamp"].transform("count").astype(np.int32)
    )
    df["Receiver_Tx_Count"] = (
        df.groupby(receiver_col)["Timestamp"].transform("count").astype(np.int32)
    )

    avg_amt = df.groupby(sender_col)["Amount Paid"].transform("mean").fillna(1.0)
    df["Amount_vs_Sender_Avg"] = (df["Amount Paid"] / (avg_amt + 1e-4)).astype(
        np.float32
    )
    ...
    print(
        f"Train positives: {sum(y_train)}, Val positives: {sum(y_val)}, Test positives: {sum(y_test)}"
    )

    # 5. Train CatBoostClassifier
    print(
        "[5/6] Training CatBoostClassifier (depth=8, auto_class_weights='SqrtBalanced', grow_policy='Lossguide')..."
    )
    ...
    print(
        f"Exported: {cbm_path}, {joblib_path}, {cal_path} (Elapsed: {time.time() - t0:.1f}s)"
    )
...
def run_module_elliptic_bitcoin():
    ...
    print(
        f"Known transactions: {len(df_known):,} (Illicit: {sum(df_known['target']):,}, Licit: {len(df_known) - sum(df_known['target']):,})"
    )
    ...
    print(
        f"Train size (t<=34): {len(X_train):,} (Illicit: {sum(y_train)}), Test size (t>=35): {len(X_test):,} (Illicit: {sum(y_test)})"
    )

    # 4. Train Topological XGBClassifier
    print(
        "[4/5] Training Topological XGBClassifier (n_estimators=1800, max_depth=7, scale_pos_weight=6.0, tree_method='hist')..."
    )
...
def run_module_samld():
    ...
    amt_col = next(
        (
            c
            for c in ["Amount", "Amount Paid", "TX_AMOUNT", "amount"]
            if c in df.columns
        ),
        df.columns[4],
    )
    ...
    print(
        f"[2/5] Computing graph fan-in, fan-out, and 24h rolling velocity on ({src_col} -> {dst_col})..."
    )
    ...
    df["Rolling_24h_Velocity"] = (
        df.groupby(src_col)[amt_col].transform("sum").astype(np.float32)
    )
    ...
    print(
        "[4/5] Training Regularized XGBClassifier (scale_pos_weight=8.0, min_child_weight=25, gamma=3.0, max_delta_step=1)..."
    )
...
def run_module_ibm_amlsim():
    ...
    print(
        f"Loaded {len(accounts_df):,} accounts and {len(transactions_df):,} transactions."
    )

    # 2. Build Graph Topology and Node Attributes
    print("[2/6] Constructing graph edge index and node topological metrics...")
    acc_id_col = (
        "ACCOUNT_ID" if "ACCOUNT_ID" in accounts_df.columns else accounts_df.columns[0]
    )
    ...
    in_deg = (
        transactions_df.groupby("RECEIVER_ACCOUNT_ID")["TX_AMOUNT"]
        .agg(["count", "sum"])
        .rename(columns={"count": "In_Degree", "sum": "Total_Received"})
    )
    out_deg = (
        transactions_df.groupby("SENDER_ACCOUNT_ID")["TX_AMOUNT"]
        .agg(["count", "sum"])
        .rename(columns={"count": "Out_Degree", "sum": "Total_Sent"})
    )

    accounts_df = (
        accounts_df.set_index(acc_id_col)
        .join(in_deg)
        .join(out_deg)
        .fillna(0)
        .reset_index()
    ...
    if torch_available:
        try:
            ...
            edge_index = torch.tensor(
                [senders.loc[valid_mask].values, receivers.loc[valid_mask].values],
                dtype=torch.long,
            )

            num_feat_cols = ["In_Degree", "Out_Degree", "Total_Received", "Total_Sent"]
            x_tensor = torch.tensor(
                accounts_df[num_feat_cols].values, dtype=torch.float
            )
    ...
    if not torch_available:
        ...
        if os.path.exists(source_gcn):
            ...
            print(
                f"Copied validated pre-trained GCN backbone: {source_gcn} -> {gcn_weights_path}"
            )
        else:
    ...
    print(
        "[5/6] Training LightGBM Classifier Head (n_estimators=1000, learning_rate=0.04)..."
    )
    n = len(X_concat)
    i_split = int(0.80 * n)
    ...
    print(
        f"Exported: {gcn_weights_path} and {lgbm_path} (Elapsed: {time.time() - t0:.1f}s)"
    )
...
def run_module_timeseries_aml():
    ...
    acc_col = (
        "time_series_ids"
        if "time_series_ids" in df.columns
        else "source_id"
    ...
    print(
        "[3/7] Engineering EWMA volume ratios, cadence delta_t, and cyclical encodings..."
    )
    df["EMA_1h"] = (
        df.groupby(acc_col)[amt_col]
        .transform(lambda s: s.ewm(span=3, adjust=False).mean())
        .astype(np.float32)
    )
    df["EMA_24h"] = (
        df.groupby(acc_col)[amt_col]
        .transform(lambda s: s.ewm(span=24, adjust=False).mean())
        .astype(np.float32)
    )
    df["EMA_7d"] = (
        df.groupby(acc_col)[amt_col]
        .transform(lambda s: s.ewm(span=168, adjust=False).mean())
        .astype(np.float32)
    )

    df["Velocity_Ratio_Short"] = (df["EMA_1h"] / (df["EMA_24h"] + 1e-4)).astype(
        np.float32
    )
    df["Velocity_Ratio_Long"] = (df["EMA_1h"] / (df["EMA_7d"] + 1e-4)).astype(
        np.float32
    )
    ...
    print(
        "[5/7] Training Model A: XGBClassifier (n_estimators=2000, max_depth=8, scale_pos_weight=1.0)..."
    )
    ...
    print(
        "[6/7] Training Model B: CatBoostClassifier (iterations=2500, depth=7, grow_policy='Lossguide')..."
    )
...
def main():
    ...
    print("\n" + "=" * 80)
    print(
        f" ALL 5 MODULES COMPLETED SUCCESSFULLY IN {total_elapsed / 60.0:.2f} MINUTES"
    )
    print("=" * 80)
    print("Serialized Artifacts Verified in models/:")
    print(
        f"  * models/ibm_transactions/catboost_model.cbm: {os.path.exists('models/ibm_transactions/catboost_model.cbm')}"
    )
    print(
        f"  * models/elliptic/xgboost_model.joblib:       {os.path.exists('models/elliptic/xgboost_model.joblib')}"
    )
    print(
        f"  * models/samld/xgboost_model.joblib:          {os.path.exists('models/samld/xgboost_model.joblib')}"
    )
    print(
        f"  * models/ibm_amlsim/gcn_backbone.pt:          {os.path.exists('models/ibm_amlsim/gcn_backbone.pt')}"
    )
    print(
        f"  * models/ibm_amlsim/lgbm_head.joblib:         {os.path.exists('models/ibm_amlsim/lgbm_head.joblib')}"
    )
    print(
        f"  * models/timeseries/xgb_ts.joblib:            {os.path.exists('models/timeseries/xgb_ts.joblib')}"
    )
    print(
        f"  * models/timeseries/cb_ts.cbm:                {os.path.exists('models/timeseries/cb_ts.cbm')}"
    )
    print("=" * 80)

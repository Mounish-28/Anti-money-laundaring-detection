...
def run_integration_test():
    ...
    try:
        ...
        print(f"  [OK] Model registry initialized ({len(engine.models)} models loaded) in {init_latency:.2f}ms")

        # Verify critical models exist
        assert "ibm_transactions" in engine.models, "IBM Transactions model missing from engine."
        assert "elliptic" in engine.models, "Elliptic Bitcoin model missing from engine."

        t0_warmup = time.perf_counter()
        engine.warmup()
        warmup_latency = (time.perf_counter() - t0_warmup) * 1000.0
        print(f"  [OK] engine.warmup() executed cleanly with zero errors in {warmup_latency:.2f}ms")
    ...
    try:
        ...
        # Assertions
        assert ibm_res.get("dataset") == "IBM Transactions", "Invalid dataset identifier"
        assert "risk_tier" in ibm_res and ibm_res["risk_tier"] in {"LOW", "ELEVATED", "HIGH", "CRITICAL_SAR"}, \
            f"Invalid risk_tier: {ibm_res.get('risk_tier')}"
        assert ibm_res.get("latency_ms", 999) < 50.0, f"IBM latency breached SLA (<50ms): {ibm_res.get('latency_ms')}ms"
        assert wall_latency_ibm < 50.0, f"IBM wall latency breached SLA (<50ms): {wall_latency_ibm}ms"

        print(f"  [PASS] IBM scoring verified within SLA (< 50ms).")
    ...
    try:
        ...
        print(f"  Elliptic Scoring Result (Raw 166 Tensor):")
        ...
        # Assertions
        assert crypto_res.get("dataset") == "Elliptic Bitcoin", "Invalid dataset identifier"
        assert "risk_tier" in crypto_res and crypto_res["risk_tier"] in {"LOW", "ELEVATED", "HIGH", "CRITICAL_SAR"}, \
            f"Invalid risk_tier: {crypto_res.get('risk_tier')}"
        assert crypto_res.get("latency_ms", 999) < 50.0, f"Elliptic latency breached SLA (<50ms): {crypto_res.get('latency_ms')}ms"
        assert wall_latency_crypto < 50.0, f"Elliptic wall latency breached SLA (<50ms): {wall_latency_crypto}ms"
        ...
        print(f"  [PASS] Elliptic 166-feature scoring verified within SLA (< 50ms).")
